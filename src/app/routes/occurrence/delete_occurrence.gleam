...
pub fn handle_request(
 ...
 id occurrence_id_str: String,
  ...
  case try_delete_occurrence(req, ctx, occurrence_id_str) {
    Ok(deleted_id) ->
      wisp.ok()
      |> wisp.set_body(wisp.Text(json.object([#("deleted_id", json.string(uuid.to_string(deleted_id)))]) |> json.to_string()))
    Error(err) ->
...
fn try_delete_occurrence(
  ...
  id_str: String,
    ...
    use _user_uuid <- result.try( // Armazenamos o UUID do usuário caso precisemos para autorização
...
fn handle_error(_req: wisp.Request, err: DeleteOccurrenceError) -> wisp.Response {
  case err {
    InvalidUuid(id) ->
      wisp.bad_request("ID de ocorrência inválido: " <> id)
    AuthenticationError(auth_err) ->
      user.handle_authentication_error(auth_err) 
      DataBaseError(db_err) ->
      database.handle_database_error(db_err) 
